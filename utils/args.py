...
def _get_validation_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--validation_driving_videos",
        type=str,
        default=None,
        help="One or more driving video path(s)/URLs that is used during validation to verify that the model is learning. Multiple validation paths should be separated by the '--validation_prompt_seperator' string. These should correspond to the order of the validation prompts.",
    )
    parser.add_argument(
...
def _get_training_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether or not to use pose control information"
    )
    parser.add_argument(
        "--pose_control_function",
        type=str,
        default="expert_layer_norm",
        help="pose control function: direct_adding; expert_layer_norm; cross_attention",
    )
