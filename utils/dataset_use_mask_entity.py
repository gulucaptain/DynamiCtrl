...
class VideoDataset(Dataset):
    def __init__(
        ...
        enable_control_pose: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.enable_control_pose = enable_control_pose
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors: # False
            ...
            image, video, control_video, _ = self._preprocess_video(self.video_paths[index])
            prompt = self.prompts[index].split("#####")[1]
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def get_entity_with_mask_short_time(self, video, masked_video):
        video = video.detach().numpy()
        masked_video = masked_video.detach().numpy()

        threshold = 200
        output_frames = []

        if video.shape[1] != masked_video.shape[1] or video.shape[2] != masked_video.shape[2]:
            return video

        frame_length, frame_height, frame_width, _ = video.shape

        for i in range(frame_length):
            video_frame = video[i]
            mask_frame = masked_video[i]
            mask_frame = mask_frame[:, :, 0]

            mask_frame = mask_frame.astype(np.uint8)
            binary_mask = (mask_frame > threshold).astype(np.uint8)
            binary_mask = np.expand_dims(binary_mask, axis=-1)

            masked_frame = video_frame * binary_mask
            output_frames.append(masked_frame)

        return torch.tensor(np.array(output_frames))
    ...
    def _preprocess_video(self, path: Path) -> torch.Tensor:
        if self.load_tensors:
            ...
            if use_image_mask:
                try:
                    mask_reader = decord.VideoReader(uri=path.as_posix().replace("videos", "mask_video"))
                except:
                    mask_reader = None
            
            if self.enable_control_pose:
                pose_reader = decord.VideoReader(uri=path.as_posix().replace("videos", "poses"))
            else:
                pose_reader = None
            ...
            frames_load_interval = 1
            frame_indices = []
            frame_index = random.randint(1, 30)
            while len(frame_indices) < nearest_frame_bucket:
                frame_indices.append(frame_index)
                frame_index += frames_load_interval
            
            # handle video frames.
            frames = video_reader.get_batch(frame_indices)
            if use_image_mask and mask_reader is not None:
                masked_frames = mask_reader.get_batch(frame_indices)
                masked_frames = self.get_entity_with_mask_short_time(frames, masked_frames) #FIXME use mask

                maked_frames = masked_frames[:nearest_frame_bucket].float()
                maked_frames = masked_frames.permute(0, 3, 1, 2).contiguous()
            ...
            if use_image_mask and mask_reader is not None:
                maked_frames_resized = torch.stack([resize(mask_frame, nearest_res) for mask_frame in maked_frames], dim=0)
                maked_frames = torch.stack([self.video_transforms(mask_frame) for mask_frame in maked_frames_resized], dim=0)

            pose_frames = pose_reader.get_batch(frame_indices)
            pose_frames = pose_frames[:nearest_frame_bucket].float()
            pose_frames = pose_frames.permute(0, 3, 1, 2).contiguous()

            pose_frames_resized = torch.stack([resize(frame, nearest_res) for frame in pose_frames], dim=0)
            pose_frames = torch.stack([self.video_transforms(frame) for frame in pose_frames_resized], dim=0)
            
            if use_image_mask and mask_reader is not None:
                image = maked_frames[:1].clone() if self.image_to_video else None
            else:
                image = frames[:1].clone() if self.image_to_video else None

            return image, frames, pose_frames, None
    ...
    def __iter__(self):
        for index, data in enumerate(self.data_source):
            ...
            if len(self.buckets[(f, h, w)]) == self.batch_size:
                if self.shuffle: # self.drop_last=False; self.shuffle=True;
